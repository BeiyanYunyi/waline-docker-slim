 ...
 * mysql Schema
 */
export default class MysqlSchema extends Schema {
  _getItemSchemaValidate(fieldData) {
    ...
    switch (fieldData.tinyType) {
      ...
      case 'int':
        validate.int = {
          min: fieldData.unsigned ? 0 : -2147483648,
          max: 2147483647,
        };
        break;
      case 'date':
  ...
  _parseItemSchema(item) {
    ...
    item.tinyType = (
      pos === -1 ? item.type : item.type.slice(0, pos)
    ).toLowerCase();
    if (
      item.default &&
      !helper.isFunction(item.default) &&
      item.tinyType.indexOf('int') > -1
    ) {
  ...
  getSchema(table = this.table) {
    ...
    return debounce.debounce(`getTable${table}Schema`, () => {
      ...
      const indexesPromise = this.query
        ...
        .then((list) => {
          ...
          const promises = list.map((item) => {
            ...
            return this.query
              .query(`PRAGMA index_info( ${item.name} )`)
              .then((data) => {
                data.forEach((item) => {
      ...
      return Promise.all([fieldsPromise, indexesPromise]).then(
        ([fields, indexes]) => {
          let ret = {};
          fields.forEach((item) => {
  ...
  parseType(tinyType, value) {
    if (tinyType === 'enum' || tinyType === 'set' || tinyType === 'bigint')
      return value;
    if (tinyType.indexOf('int') > -1) return parseInt(value, 10);
    if (['double', 'float', 'decimal'].indexOf(tinyType) > -1)
